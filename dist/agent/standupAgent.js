...
exports.StandupAgent = void 0;
const uuid_1 = require("uuid");
const moment_1 = __importDefault(require("moment"));
class StandupAgent {
    constructor(dataStorage, bedrockService, maxQuestions = 5) {
        this.dataStorage = dataStorage;
        this.bedrockService = bedrockService;
        this.maxQuestions = maxQuestions;
    }
    async processStandupEntry(rawNotes, memberName) {
    ...
    async answerQuestion(questionId, answer) {
        ...
        const entries = await this.dataStorage.getEntriesByMember(this.extractMemberIdFromEntry(question.entryId), 7);
        const followUpQuestions = await this.bedrockService.generateFollowUpQuestions(question, answer, entries);
        if (followUpQuestions.length > 0) {
            await this.saveQuestions(followUpQuestions);
        }
        return followUpQuestions;
    }
    async findOrCreateMember(memberName) {
    ...
    async parseStandupNotes(rawNotes, member) {
        const entry = {
            id: (0, uuid_1.v4)(),
            memberId: member.id,
    ...
    generateSummary(entry) {
        const parts = [];
        if (entry.yesterday)
            parts.push(`Completed: ${entry.yesterday.substring(0, 50)}...`);
        if (entry.today)
            parts.push(`Working on: ${entry.today.substring(0, 50)}...`);
        if (entry.blockers)
            parts.push(`Blocked by: ${entry.blockers.substring(0, 30)}...`);
        return parts.join(' | ') || 'Standup update';
    }
    async analyzeStandupEntry(currentEntry) {
...
exports.StandupAgent = StandupAgent;
//# sourceMappingURL=standupAgent.js.map